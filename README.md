...
Using [Algolia](https://www.algolia.com/) (a cool, blazing fast search engine) and [InstantSearch](https://www.algolia.com/doc/guides/building-search-ui/what-is-instantsearch/js/) (UI library to make search interfaces ). Alongside [NextJS](https://nextjs.org/) (cutting edge full-stack JavaScript framework that many people love), we are gonna pick some characters from a videogame (Genshin Impact) and display them in a fancy way, being able to click around, filter, order and explore many of the features that InstantSearch and Algolia offer to give our users the best searching experience.
...
Here, we'll be able to create our index, which stores our data. I'll create mine with the name `characters`, as you can see it in the picture. Hit on `Create Index`, and wait until `Import your records` is available.
...
I want you to check for a second at the hooks. They will always provide useful state information and functions to change it. Please, remember to check the documentation for every component on the widget showcase, everything will be explained over there, and you'll learn why `refine` is going to be one of your best allies.
